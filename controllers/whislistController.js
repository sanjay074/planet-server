...
const getWhishList = async (req, res) => {
  ...
  const myWhilist = await Whislist.findOne({userId}).populate('whislistItems.productId','name description finalPrice basePrice images size numSize footSize discountPrice')
    if(!myWhilist){
    return res.status(400).send({
      success:false,
